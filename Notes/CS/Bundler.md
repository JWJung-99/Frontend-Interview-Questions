...
번들러는 여러 개의 JavaScript, CSS, 이미지 파일 등을 하나 또는 최적의 소수 파일로 결합하는 도구입니다. 번들러는 모듈 시스템을 이용해 여러 개의 파일을 묶거나 최적화하여 웹 애플리케이션의 성능을 향상시키는데 주로 사용됩니다.

주요 기능으로는 **모듈 번들링**, **코드 스플리팅**, **트리 쉐이킹**, **파일 최적화** 등이 있습니다. 주로 사용되는 번들러로는 Webpack, Parcel, Rollup, Vite, ESbuild 등이 있습니다.

<br />

### 빌드 도구

빌드 도구는 소스 코드를 최종적으로 실행 가능한 형태로 변환하는 프로세스를 자동화하는 도구입니다. 빌드 도구의 주요 기능으로는 **트랜스파일링**, **파일 최적화**, **자동화**, **리소스 변환** 등이 있습니다. 대표적인 빌드 도구로 Webpack, Vite, Gulp, Grunt 등이 있습니다.

<br />

| 항목               | **빌드 도구 (Build Tool)**                                | **번들러 (Bundler)**                          |
|--------------------|----------------------------------------------------------|----------------------------------------------|
| **주요 기능**      | 전체 빌드 프로세스를 관리하고, 트랜스파일링, 최적화, 파일 변환 등 다양한 작업을 처리 | 여러 개의 파일을 하나의 번들로 묶거나 코드 최적화 및 분할 처리 |
| **사용 목적**      | 코드 최적화, 트랜스파일링, 자동화된 작업 처리              | 여러 파일을 하나의 번들로 묶어서 성능 향상     |
| **대표적인 도구**  | Webpack, Vite, Gulp, Grunt                               | Webpack, Parcel, Rollup, Vite                |
| **핵심 역할**      | 전체 빌드 작업 자동화 및 최적화                          | 모듈 시스템을 통해 코드 묶기 및 최적화 처리    |
| **사용 예시**      | Babel을 사용해 최신 JavaScript 코드를 트랜스파일하거나, Gulp로 Sass를 CSS로 컴파일하는 작업 | 여러 JavaScript 모듈을 하나로 묶거나, 코드 스플리팅으로 로딩 성능 향상 |
